    ...
    public class WorkshopController : ControllerBase
    {
        ...
        public IActionResult GetWorkshops()
        {
            ...
            try
            {
                ...
                var presentations = databaseContext.Presentations;

                foreach (tblWorkshops entry in workshops.ToList())
                {
                    tblUsers organizer = users.Where(x => x.Id == entry.OrganizerId).First();
                    tblPresentation presentation = presentations.Where(x => x.Id == entry.PresentationId).First();
        ...
        public IActionResult CrateWorkshop([FromBody] tblWorkshops request)
        {
            ...
            workshop.PresentationId = request.PresentationId;
        ...
        [Route("CreatePresentation")]
        [HttpPost]
        public IActionResult CreatePresentation([FromBody] tblPresentation presentation) {

            try
            {
                if (databaseContext.Presentations.FirstOrDefault(x => x.Name == presentation.Name && x.Duration == presentation.Duration) == null)
                {
                    databaseContext.Presentations.Add(presentation);
                    databaseContext.SaveChanges();
                }
            ...
            int presentationId = databaseContext.Presentations.FirstOrDefault(x => x.Name == presentation.Name && x.Duration == presentation.Duration).Id;
            return StatusCode(200, presentationId);
        ...
        [HttpPut]
        public IActionResult UpdatePresentation([FromBody] tblPresentation request)
        {
            try
            {

                    var presentation = databaseContext.Presentations.FirstOrDefault(x => x.Id == request.Id);
                    if (presentation != null)
                    {
                        presentation.Name = request.Name;
                        presentation.Duration = request.Duration;   

                        databaseContext.Entry(presentation).State = EntityState.Modified;
                        databaseContext.SaveChanges(true);

                        return StatusCode(200, presentation.Id);
                    }
                    else
                    {
                        return StatusCode(404, "Prezentacija nije pronadjena");
                    }
            }
            catch (Exception e)
            {
